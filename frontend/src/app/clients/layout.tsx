...
import {
  ...
  Phone, Receipt, Zap, MessageSquare, BookOpen, MessageSquareText
} from 'lucide-react';
...
function ClientLayoutInner({
  ...
  useEffect(() => {
    ...
    if (storageConsent === 'granted') {
      ...
      console.log('Client theme initialized to:', currentDOMTheme, '(consent granted, from DOM)');
    } else if (storageConsent === 'denied') {
      setHasStorageConsent(false);
      // Use the theme from DOM but don't save
      setTheme(currentDOMTheme);
      setPendingTheme(currentDOMTheme);
      console.log('Client theme initialized to:', currentDOMTheme, '(consent denied, from DOM)');
    } else {
  ...
  useEffect(() => {
    if (pathname === '/clients/messages') {
      markMessagesAsRead();
    }
    if (pathname === '/clients/appointments') {
  ...
  const toggleTheme = () => {
    ...
    if (storageConsent === 'granted') {
      ...
      console.log('Client theme changed to:', newTheme, '- saved to localStorage');
    } else {
      // No consent yet OR consent denied - always show modal
      setShowConsentModal(true);
      console.log('Client theme changed to:', newTheme, '- showing consent modal');
    }
  };
  ...
  return (
        ...
        {!isInMeeting && (
          ...
          <div className="flex h-full flex-col bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide min-h-0 flex flex-col" data-tour="navigation">
              ...
              <div className="flex items-center h-16 px-4">
                {(() => {
                  ...
                  const sortedItems = allNavItems.sort((a, b) => {
                    if (a.href === '/clients') return -1; // Dashboard always first
                    if (b.href === '/clients') return 1;
