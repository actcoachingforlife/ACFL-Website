  ...
  useEffect(() => {
    ...
    const checkAndCollapseForModal = () => {
      ...
      const isExcludedDropdown = (element: Element): boolean => {
        // Check if element or its parents have notification/profile dropdown classes
        ...
        while (current) {
          ...
          if (typeof classes === 'string') {
            ...
            // Check if it's inside these refs
  ...
  return (
    <ProtectedRoute allowedRoles={['coach']}>
      <div className={`min-h-screen bg-gray-50 dark:bg-gray-900 flex overflow-x-hidden ${poppins.className}`}>
        ...
        {/* Main Content Area */}
        <div className={`flex-1 ${isInMeeting ? '' : (sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-72')} transition-all duration-500 ease-out relative z-[1] min-w-0 overflow-x-hidden ${!isInMeeting ? 'lg:pt-16' : ''}`}>
          {/* Mobile Header */}
          {!isInMeeting && (
